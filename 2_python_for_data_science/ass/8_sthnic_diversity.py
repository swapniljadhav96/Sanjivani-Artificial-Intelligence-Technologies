# -*- coding: utf-8 -*-
"""
Created on Sun Mar 10 12:17:16 2024
...
#############################################################
#2nd file data set "C:\Data Science\1-python\ethnic diversity.csv"
import pandas as pd
df=pd.read_csv("C:/Data Science/1-python/ethnic diversity.csv")
df
...
df = df.astype({"Zip": float, "age": float})
print(df.dtypes)
...
#####################
#raise error
df = df.astype({"EmpID": int},errors='raise')
df
############################
...
df2 = df.rename({'age': 'Age', 'EmpID': 'eid'}, axis='columns')
df2
df2 = df.rename(columns={'age': 'AGE', 'EmpID': 'EID'})
df2
df=df.rename(columns={'EmpID': 'ID', 'age': 'AGE@'},inplace=True)
df
